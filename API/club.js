...
const filePath = path.join(__dirname,'../json/clubFile.json');
...
const clubData = {
  ...
  following: edge_follow.count,
    ...
    clubData.images = extractedData;
    console.log(clubData.images);
    ...
    clubData.postLikes = extractedData.map((ele) => ele.likes);
    clubData.postDataUrl = extractedData.map((ele) => ele.link); // Use the encoded URLs here
    clubData.postDataAvgLikes = parseInt(sum.toString());

    return clubData; // Return the data
  } catch (error) {
